...
const minRed = (test: string): number => {
  const redTest = /(\d+) red/g
  return Math.max(1, ...[...test.matchAll(redTest)].map(([_, a]) => parseInt(a)))
}
const minGreen = (test: string): number => {
  const greenTest = /(\d+) green/g
  return Math.max(1, ...[...test.matchAll(greenTest)].map(([_, a]) => parseInt(a)))
}
const minBlue = (test: string): number => {
  const blueTest = /(\d+) blue/g
  return Math.max(1, ...[...test.matchAll(blueTest)].map(([_, a]) => parseInt(a)))
}
